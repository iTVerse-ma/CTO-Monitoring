...
export interface Job {
  ...
  instance: string;
  name: string;
  label: string;       // pack label (display)
  payload: any;        // create params — SERVER-SIDE ONLY, never projected to clients
  status: JobStatus;
  step: string;
  pct: number;
  result: any;
...
const queues = new Map<string, string[]>(); // instance -> [jobId, ...] (live array; drain shifts, enqueue pushes)
const active = new Set<string>();           // instances currently draining
let runner: Runner | null = null;
const MAX_JOBS = 200;

export function setRunner(r: Runner) { runner = r; }

// True if a not-yet-finished job already targets this instance+name (guards double-submit).
...
export function enqueue(input: { instance: string; name: string; label: string; payload: any }): Job {
  ...
  void drain(job.instance); // fire-and-forget — do NOT await (that would block the request)
  return job;
}

async function drain(instance: string) {
  if (active.has(instance)) return; // already draining this instance serially
  active.add(instance);
  try {
    ...
    const q = queues.get(instance) || [];
    if (q.length) void drain(instance); // a job enqueued during the last await
  }
}
...
// Browser-safe projection — NEVER includes payload. result is the runner's own
// (already client-safe) return value, surfaced only once the job is done.
