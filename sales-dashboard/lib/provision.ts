// The actual tenant provisioning flow, run by the background queue pump (lib/queue).
// Extracted from the old synchronous create route so it can report progress via
// onStep(). SALES-SAFE: the returned result carries only the CLIENT's own login +
// password — never Connecteo's shared recovery/master credentials — and on failure
// it does NOT fall back to the shared admin (mirrors the original sales policy).
...
export async function provisionSales(job: Job, onStep: (step: string, pct: number) => void) {
  const b = job.payload || {};
  const packDef = getPack(b.pack) || getPack('start');
  if (!packDef) throw new Error('aucun pack configuré');
  ...
  let client: any = null, adminWarning: string | null = null;
  if (!moduleWarning) {
  ...
  return {
    ...
    client, installed, extraUsers, extraDepots,
